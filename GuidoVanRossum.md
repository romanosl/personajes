# Guido van Rossum


Guido van Rossum es el creador de Python, uno de los mayores lenguajes de programación en la web y fuera de esta. La comunidad Python se refiere a e’l como un dictador benevolente para la vida. Él cambio de residencia de Holanda a Estados Unidos en 1995, en donde conoció a su esposa. Hasta julio de 2003 vivieron en el noroeste de Virginia, suburbios de Washington, DC, con su hijo Orlijn, quien nació en el 2001. Después se mudaron a Silicon Valley en donde Guido trabajo para diferentes compañías, incluyendo Google y Dropbox. Pasaba el 50% de su tiempo trabajando en Python.
En enero de 2013 se unió al equipo de Dropbox, trabajó en varios productos. Anteriormente trabajo para Google, Elemental Security, Zope Corporation, BeOpen.com, CNRI and SARA. Creo Python mientras estaba en CWI.
Guido van Rossum tiene una maestría en Matematicas y Ciencias Computacionales (University of Amsterdam, 1982) y se unio a CWI como investigador ese mismo año. Ha obtenido los siguientes premios:

* En Junio de  2013 Python gano el altamente competitivo: Dutch COMMIT/ Award.
* En Julio de 2007 fue premiado por  la USENIX: STUG Award.
* En Octubre de 2006 fue elegido como ACM Distinguished Engineer.
* En Junio de 2003 fue finalista "IT - Software (Individual)" del World Technology Network awards.
* En Mayo de 2003 recibió el NLUUG Award 2003 por los servicios extraordinarios a la comunidad de usuarios de Unix y Open Systems.
* En Febrero de 2002 recibió el Free Software Foundation Award.
* En Mayo de 1999 recibio el Dr. Dobb's Journal 1999 Excellence in Programming Award, junto  con Donald Becker.

Guido van Rossum ha hecho diferentes publicaciones con los siguientes temas:

##### Python
* Guido van Rossum: Scripting the Web with Python. In "Scripting Languages: Automating the Web", World Wide Web Journal, Volume 2, Issue 2, Spring 1997, O'Reilly.
* Aaron Watters, Guido van Rossum, James C. Ahlstrom: Internet Programming with Python. MIS Press/Henry Holt publishers, New York, 1996.
* Guido van Rossum: Python Library Reference. May 1995. CWI Report CS-R9524.
* Guido van Rossum: Python Reference Manual. May 1995. CWI Report CS-R9525.
* Guido van Rossum: Python Tutorial. May 1995. CWI Report CS-R9526.
* Guido van Rossum: Extending and Embedding the Python Interpreter. May 1995. CWI Report CS-R9527.
* Guido van Rossum, Jelke de Boer: Linking a Stub Generator (AIL) to a Prototyping Language (Python). Spring 1991 EurOpen Conference Proceedings (May 20-24, 1991) Tromso, Norway.

##### Multimedia
* An Interactive Multimedia Management Game, by Lynda Hardman, Guido van Rossum and Aart van Bolhuis. (Journal of Intelligent Systems, 5:151-177, 1995)
* Links in Hypermedia: the Requirement for Context, by Lynda Hardman, Dick C.A. Bulterman and Guido van Rossum (ACM Hypertext '93, Seattle WA, Nov '93, 183-191)
* Structured Multimedia Authoring, by Lynda Hardman, Guido van Rossum and Dick C A Bulterman (ACM Multimedia '93, Anaheim, Aug '93, 283 - 289) (revised version of CWI Report CS-R9304)
* CMIFed: A Presentation Environment for Portable Hypermedia Documents, by Guido van Rossum, Jack Jansen, K.Sjoerd Mullender and Dick C.A. Bulterman (ACM Multimedia '93, Anaheim, Aug '93, 183 - 188) (revised version of CWI Report CS-R9305)
* The Amsterdam Hypermedia Model: extending hypertext to support real multimedia, by Lynda Hardman, Dick C A Bulterman and Guido van Rossum (Hypermedia Journal 5(1), July 1993, 47 - 69) (revised version of CWI Report CS-R9306)
* Dick C.A. Bulterman, Guido van Rossum, Robert van Liere: A Structure for Transportable, Dynamic Multimedia Documents. Proceedings of the Summer 1991 USENIX Conference, Nashville, Tennessee, USA (June 10-14, 1991), pp. 137-155.
* Dick C.A. Bulterman, Guido van Rossum, Dik Winter: Multimedia Synchronization and UNIX -or- If Multimedia Support is the Problem, Is UNIX the Solution? Autumn 1991 EurOpen Conference Proceedings, Budapest, Hungary (September 16-20, 1991), pp. 29-45.

##### STDWIN
* G. van Rossum: STDWIN - A Standard Window System Interface. CWI Report CS-R8817 (April 1988).

##### Amoeba
* S.J. Mullender, G. van Rossum, R. van Renesse, J.M. van Staveren: Amoeba: A Distributed Operating System for the 1990s. IEEE Computer Magazine vol. 23, no. 5 (May 1990), pp. 44-53.
* A.S. Tanenbaum, R. van Renesse, J.M. van Staveren, G.J. Sharp, S.J. Mullender, A.J. Jansen, G. van Rossum: Experiences with the Amoeba Distributed Operating System. CACM vol. 33, no. 12 (December 1990), pp 46-63.
* G. van Rossum: AIL - A Class-Oriented Stub Generator for Amoeba. Workshop on Progress in Distributed Operating Systems and Distributed Systems Management (eds.: W. Schroder-Preikschat and E.W. Zimmer), Lecture Notes in Computer Science, Springer Verlag, vol. 433 (1990), pp 13-21.

##### ABC
* L.G.L.T. Meertens, S. Pemberton, and G. van Rossum: The ABC structure editor. Structure-based editing for the ABC programming environment. CWI report no. CS-R9256 (1992).

Guido van Rossum ha sido uno de los personajes que ha influenciado al software libre al inventar e implementar Python como un lenguaje de programación de software libre.

## ¿Qué es Python?
Python es un lenguaje de programación de software libre poderoso y fácil de aprender. Cuenta con estructuras de datos eficientes y de alto nivel y un enfoque simple pero efectivo a la programación orientada a objetos. La elegante sintaxis de Python y su tipado dinámico, junto con su naturaleza interpretada, hacen de éste un lenguaje ideal para scripting y desarrollo rápido de aplicaciones en diversas áreas y sobre la mayoría de las plataformas. 
El intérprete de Python y la extensa biblioteca estándar están a libre disposición en forma binaria y de código fuente para las principales plataformas desde el sitio web de Python, http://www.python.org/, y puede distribuirse libremente. El mismo sitio contiene también distribuciones y enlaces de muchos módulos libres de Python de terceros, programas y herramientas, y documentación adicional. El intérprete de Python puede extenderse fácilmente con nuevas funcionalidades y tipos de datos implementados en C o C++ (u otros lenguajes accesibles desde C). Python también puede usarse como un lenguaje de extensiones para aplicaciones personalizables.

Ejemplo de código Python: Hello World!
````Python
print 'Hello, world!'
````
Ejemplo de código Python: Input, assignment 
````python
name = raw_input('What is your name?\n')
print 'Hi, %s.' % name
````
### Referencias bibliográficas 
* https://www.python.org/~guido/bio.html 
* https://www.python.org/~guido/Resume.html 
* https://www.python.org/~guido/Publications.html
* https://www.python.org/doc/essays/blurb/
* http://docs.python.org.ar/tutorial/pdfs/TutorialPython2.pdf
